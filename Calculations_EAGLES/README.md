...
6. **`6_austrittsgeschwindigkeit.png`** — Luftgeschwindigkeit am Rotoraustritt
   (Slipstream, `V_jet = V + 2w`) über der Fluggeschwindigkeit im stationären
   Horizontalflug (T = Widerstand). Referenzlinie `V_jet = V` (kein Schub); der
   Abstand ist der Geschwindigkeitsüberschuss. Größerer Durchmesser → kleinerer
   Überschuss → höherer Froude-Wirkungsgrad.
